            ...
            <div class="dropdown-menu" v-loading="loading || switchingNode">
                ...
                <div v-if="showNodes()">
                    <el-scrollbar max-height="168px" :noresize="true">
                        <div
                            class="dropdown-item"
                            @click="changeNode(item.value || item.name)"
...
import { listNodes, setDefaultNodeInfo } from '@/utils/node';
...
const nodes = ref<Array<Setting.NodeItem>>([]);
const nodeOptions = ref<Array<Setting.NodeItem>>([]);
const loading = ref();
...
const loadCurrentName = () => {
    if (currentNode.value) {
        ...
        const item = nodes.value.find(
            (node) => (node.value || node.name) === currentNode.value || node.name === currentNode.value,
        );
        if (item) {
            return loadNodeName(item);
        }
...
const loadNodeName = (item: Setting.NodeItem) => {
...
const changeFilter = () => {
    ...
    for (const item of nodes.value) {
        if (loadNodeName(item).indexOf(filter.value) !== -1) {
...
const changeNode = async (command: string) => {
    ...
    try {
        for (const item of nodes.value) {
            const nodeValue = item.value || item.name;
            if (nodeValue == command) {
                if (command == 'local') {
...
const loadGlobalSetting = async (currentNode?: string) => {
    try {
        const res = await getAgentSettingInfo(currentNode);
        if (res.data?.defaultNetwork) {
...
const showNodes = () => {
    return isXpackOrEE.value || nodes.value.length > 1;
};
