...
func Proxy() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if service.IsOpenNodeName(currentNode) {
			if err := service.NewIOpenNodeService().Proxy(c, currentNode); err != nil {
				helper.ErrorWithDetail(c, http.StatusBadGateway, "ErrProxy", err)
			}
			return
		}
